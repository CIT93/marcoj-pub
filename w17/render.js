...
const renderTblHeading = function() {
    ...
    const headingTextArr = ["First Name", "Last Name", "Footprint Total", "Action"];
...
const boxesOnEdit= function(obj){
    if (obj.washerChecked === true){
        FORM.toggleWasher.checked = true;
        document.getElementById("washerContainer").style.display = "block";
    }
    if (obj.dishesChecked === true){
        FORM.toggleDishWasher.checked = true;
        document.getElementById("dishWasherContainer").style.display = "block";
    }
}

const recycleButtonsonEdit = function(obj){
    if (obj.glass === true){
        FORM.cGlass.checked = true
    }
    if (obj.plastic === true){
        FORM.cPlastic.checked = true
    }
    if (obj.paper === true){
        FORM.cPaper.checked = true
    }
    if (obj.aluminum === true){
        FORM.cAluminum.checked = true
    }
    if (obj.steel === true){
        FORM.cSteel.checked = true
    }
    if (obj.food === true){
        FORM.cFood.checked = true
    }
}
